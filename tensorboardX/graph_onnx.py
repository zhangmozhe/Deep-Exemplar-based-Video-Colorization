...
def gg(fname):
    ...
    for node in nodes_proto:
        ...
        nodes.append(
            NodeDef(
                ...
                op="Variable",
                input=[],
                attr={"dtype": AttrValue(type=node.type.tensor_type.elem_type), "shape": AttrValue(shape=shapeproto),},
            )
        )
    ...
    for node in g.node:
        ...
        nodes.append(
            NodeDef(name=node.output[0], op=node.op_type, input=node.input, attr={"parameters": AttrValue(s=attr)},)
        )
